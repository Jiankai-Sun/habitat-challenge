...
def construct_envs(args):
    ...
    # basic_config = cfg_env(args.task_config)
    ...
    for update in range(args.num_updates):
        ...
        if done:
            ...
            for sensor in rollouts.observations:
                rollouts.observations[sensor][0] = batch[sensor]
            rollouts.to(device)
        ...
        for step in range(args.num_steps):
            ...
            rollouts.insert(
                ...
                distribution.probs,
                masks,
                st_actions=torch.tensor(shortest_path_action),
            )

            # print(shortest_path_action, actions, distribution.probs)
            count_steps += 1

            print(distribution.probs, torch.tensor([shortest_path_action]).shape)
            # action_loss = criterion(distribution.probs, torch.tensor([shortest_path_action]).to(device))
            #
            # optimizer.zero_grad()
            ...
            # num_updates += 1
            #
            # action_loss_avg = action_loss / num_updates
        ...
        if update > 0 and update % args.log_interval == 0:
            ...
            if window_counts > 0:
                logger.info(
                    "Average window size {} reward: {:3f} action_loss_avg: {:3f}".format(
                        len(window_episode_reward),
                        (window_rewards / window_counts).item(), action_loss,
                    )
                )
